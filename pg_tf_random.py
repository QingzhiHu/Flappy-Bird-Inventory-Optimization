...
# -*- coding: utf-8 -*-
"""
Created on Sat Jan 27 20:26:20 2018
...
from q_learning_bins import plot_running_avg, FeatureTransformer
...
class HiddenLayer:
    def __init__(self, M1, M2, f=tf.nn.tanh, use_bias=True, zeros=False):
        ...
        if use_bias:
            self.b = tf.Variabe(np.zeros(M2).astype(np.float32))
...
class PolicyModel:
    def __init__(self, ft, D, hidden_layer_sizes_mean=[], hidden_layer_sizes_var=[]):
        ...
        # store inputs for reduplicating
        self.ft = ft
        self.D = D
        ...
        norm = tf.contrib.distribution.Normal(mean, var)
        ...
        def perturb_params(self):
            ...
            for p in self.params:
                ...
                if np.random.random() < 0.1:
                    # with prob 0.1 start completely from scratch
                    op = p.assign(noise)
                else:
                    op = p.assign(v + noise)
                ops.append(op)
